...
export default function MatcherMode() {
  ...
  const handleCopy = async () => {
    ...
    let textToCopy = `Core Strategy:\n${suggestion.coreSuggestion}\n\n`;
    textToCopy += `Recommended Principles (${suggestion.relevantLaws.length} Matches):\n\n`;

    suggestion.relevantLaws.forEach(item => {
      const law = getLawById(item.id);
      if (law) {
        textToCopy += `${law.emoji} ${law.title} (${law.category})\n`;
        textToCopy += `"${item.contextualUsage}"\n`;
        textToCopy += `${law.summary}\n\n`;
      }
    });
        ...
        {suggestion && !isLoading && (
          ...
          >
            <div className="flex justify-end mb-4">
              <button
                onClick={handleCopy}
                className="flex items-center gap-2 px-4 py-2 bg-white/75 backdrop-blur-sm border border-white/20 hover:bg-white/90 rounded-full text-sm font-bold text-stone-700 transition-all shadow-sm cursor-pointer"
              >
                {hasCopied ? (
                  <>
